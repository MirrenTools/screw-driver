...
public class ScrewDriverMainExample {
	...
	public static void main(String[] args) throws Exception {
		...
		templates.put("entity", new SdTemplate().setFile("entity.ftl").setSourceFolder(Constant.MAVEN_SRC).setPackageName("entity").setClassName("User"));
		templates.put("dao", new SdTemplate().setFile("dao.ftl").setSourceFolder(Constant.MAVEN_SRC).setPackageName("dao").setClassName("UserDao"));
		// 初始化代码执行生成工具
		SdCodeUtil codeUtil = new SdCodeUtilImpl(bean, templates);
		// 初始化SQL执行生成工具
		SdDatabaseOptions databaseOptions = new SdDatabaseOptions(MySQL.MYSQL_8_DERVER, "jdbc:mysql://localhost:3306/root?useUnicode=true&useSSL=false&characterEncoding=UTF-8&serverTimezone=UTC");
		databaseOptions.setUser("root");
		databaseOptions.setPassword("root");
		SdSqlUtil sqlUtil = new SdSqlUtilImpl(bean, databaseOptions);
		// 执行代码
