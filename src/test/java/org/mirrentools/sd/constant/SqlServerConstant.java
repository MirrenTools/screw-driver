...
public class SqlServerConstant {
	...
	static {
		...
		initClassContent();
		// 数据库配置信息
		databaseOptions = new SdDatabaseOptions(SqlServer.SQL_SERVER_DERVER, "jdbc:sqlserver://localhost:1433;DatabaseName=item");
		databaseOptions.setUser("sa");
	...
	private static void initBean() {
		...
		SdColumn cc1 = new SdColumn().setName("id").setType(SdType.INTEGER).setRemark("班级的id").setPrimary(true).setIdentity(true);
		...
		column6.setUnique(true).setIndexRemark("测试注释").setIndexType(SqlServer.INDEX_NONCLUSTERED).setIndexName("idx_user_mobile_phone");
