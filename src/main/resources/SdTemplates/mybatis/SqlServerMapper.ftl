...
<mapper namespace="${content.items.dao.packageName}.${content.items.dao.className}">
	...
	<select id="selectAll" parameterType="${assign_AssistFullName}" resultMap="result_entity_map">
		...
		<choose>
			...
			<otherwise>
			 	order by <#if content.content.primaryField??>${content.content.primaryField[0].name}</#if>
			</otherwise>
		</choose>
		) AS tt_row_index 
		</if>
		...
		<if test="rowSize!=null">
			...
			<choose>
				<when test="startRow!=null">
				  tt_row_index &gt; ${r'#{startRow}'} and tt_row_index &lt;= ${r'#{startRow}'}+${r'#{rowSize}'}
				</when>
				<otherwise>
				  tt_row_index &gt; 0 and tt_row_index &lt;= ${r'#{startRow}'}+${r'#{rowSize}'}
				</otherwise>
			</choose>
		</if>
	</select>
