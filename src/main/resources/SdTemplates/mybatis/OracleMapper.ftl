...
<mapper namespace="${content.items.dao.packageName}.${content.items.dao.className}">
	...
	<select id="selectAll" parameterType="${assign_AssistFullName}" resultMap="result_entity_map">
		<if test="rowSize!=null">
			select * from ( select temp_table.*, ROWNUM AS tt_row_index from (
		</if>
		select
		...
		<if test="rowSize!=null">
			) temp_table where ROWNUM &lt;=
			<if test="startRow!=null">${r'#{startRow}'}+</if>${r'#{rowSize}'}
			) tt_result_table
			where tt_result_table.tt_row_index &gt;= ${r'#{startRow}'}
		</if>
	</select>
